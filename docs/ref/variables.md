...
**Description:** The full Wazuh version string (e.g. `4.14.8`) extracted from `wazuh_version_data`.  
**Default value:** `{{ wazuh_version_data.version }}`

---

**Variable:** `wazuh_major_minor_version`  
**Description:** The major and minor version components only (e.g. `4.14`), derived from `wazuh_full_version`.  
**Default value:** `{{ wazuh_version_data.version.split('.')[0:2] | join('.') }}`

---

**Variable:** `wazuh_major_version`  
**Description:** The major version string in `X.x` format (e.g. `4.x`), used in package repository URL paths.  
